...
def safe_id(cid):
    """A catalog id is used as a directory name, so it stays a plain slug.

    Ids are generated by _slug, but the index file is editable by hand and is
    read back on every call — a doctored id must not be able to point the
    gallery at another part of the disk.
    """
...
def remove(cid, delete_files=False):
    ...
    if delete_files:
        ...
        shutil.rmtree(os.path.join(CATALOGS_DIR, safe_id(cid)), ignore_errors=True)
    return True
