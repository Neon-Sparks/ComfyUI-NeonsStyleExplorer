...
* **CI failed on `git diff --exit-code`** because the test suite saves a preview,
  the default catalog keeps its previews inside the package, and
  `previews/manifest.json` is tracked in the repository — so every run rewrote a
  tracked file. The suite now stashes that folder alongside `user/*.json` and
  restores it exactly, deleting whatever the run itself created.
* `tools/preflight.sh` now refuses **any** tracked file under `previews/` except
  its README, rather than only images. A tracked manifest is what turned a
  harmless local file into a failing build.

* **The released zip was carrying two stray preview images and a manifest** left
  over from my own testing, which is how a generated file reached the repository
  in the first place. The package now ships `previews/` with nothing in it but
  its README, as it always should have.

**If your repository still tracks it**, one command removes it without deleting
your previews:

```bash
git rm --cached previews/manifest.json
git commit -m "Stop tracking the generated preview manifest"
```
