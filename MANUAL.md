...
**An image saved onto a style it was not made with.** Fixed in 2.2.2. The style
for each prompt is recorded server-side, but ComfyUI caches a node whose inputs
have not changed, so a re-queue could execute nothing and leave that prompt
without a record — and the server then fell back to the most recent record it
